...
namespace fstim
{
    std::unique_ptr<Mesh2d> MeshFactory::operator()(Vec2i size, Vec2d length)
    {
    ...
    void MeshFactory::m_assignIds(std::unique_ptr<Cell2d[]>& cells, std::unique_ptr<Face2d[]>& faces, Vec2i size)
    { 
    ...
    void MeshFactory::m_assignProperties(std::unique_ptr<Cell2d[]>& cells, std::unique_ptr<Face2d[]>& faces, Vec2i size, Vec2d length)
    {
        int nCells = size.x * size.y;
